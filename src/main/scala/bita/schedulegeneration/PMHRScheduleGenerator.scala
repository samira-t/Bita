...
/**
 * Generates schedules for PMHR criterion.
 */
object PMHRScheduleGenerator extends ScheduleGenerator {

  // optimization MAX_INDEPENDENT is only for PR.
  //def optimizations = ScheduleOptimization.values.-(MAX_INDEPENDENT)

  logger = new Logger("PMHRSchedGen")
  logger.enable = false

  criterion = PMHRCriterion
 ...
 */
    ...
    for (trace <- traces) {
      ...
      for (i <- 0 to length - 1 if continue) {
        ...
        for (j <- i + 1 to length - 1 if continue) {
          ...
          if (criterion.satisfy(trace, i, j)) {
            ...
            if (canBeReordered(i, j, trace)) {
              ...
              if (coveredPairs.contains(originalPair) && !coveredPairs.contains(reversepair)) {
                ...
                var newTrace = reorderAndGenerate(i, j, trace, /*prefixIndex*/ i - 1, true)
                var relaxedPointIndex = newTrace.getRelaxedIndex
          ...
          continue = maxSchedule < 0 || (traceScheduleCount < maxScheduleForEachTrace && generatedTraces.size < maxSchedule)
  ...
  private def reorderTail_REORDER_TAIL(trace: Trace, tailIndex: Int): Trace = {
      ...
      for (j <- i + 1 to length - 1) {
        ...
        if (criterion.satisfy(trace, i, j)) {
          ...
          if (canBeReordered(i, j, trace)) {
            ...
            if (coveredPairs.contains(originalPair) && !coveredPairs.contains(reversePair)) {
              // generate for the ordering of reverse pair
            ...
            } else if (!coveredPairs.contains(originalPair)) {
              // generate for the ordering of pair
              var newTrace = bringCloseAndGenerate(i, j, trace, i - 1, true)
              logger.logLine(" ^^%s^^ : %s".format(newTrace.getRelaxedIndex, newTrace.getEvent(newTrace.getRelaxedIndex).toString()))
              return reorderTail_REORDER_TAIL(newTrace, newTrace.getRelaxedIndex + 1)
            } else
          ...
          // next j events won't be consecutive to i 
          consecutive = false
        }
      }
    }
     ...
     */
    //This optimization for PMHR criterion seems to make the results worse
            ...
            if (canBeReordered(i, j, trace)) {
              ...
              if (!coveredPairs.contains(originalPair)) {
                ...
                if (consecutive) {
                  ...
                  var newTrace = bringCloseAndGenerate(i, j, trace, i - 1 /*prefixIndex*/ , false)
              ...
              if (coveredPairs.contains(originalPair) && !coveredPairs.contains(reversepair)) {
                ...
                var newTrace = reorderAndGenerate(i, j, trace, /*prefixIndex*/ i - 1, false)
