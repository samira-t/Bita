...
/**
 * Criterion that considers pair of consecutive receives in each actor.
 */
object PCRCriterion extends Criterion {
  ...
  def satisfy(t: Trace, i: Int, j: Int, k: Int = 0): Boolean = {
    var ei = t.getTrace()(i)
    var ej = t.getTrace()(j)
    return (ei.receiverIDStr == ej.receiverIDStr) && (!ei.promiseResponse) && (!ej.promiseResponse)
  }
  ...
  def generateSchedules(name: String, randomTracesPath: Array[String], generatedSchedulesPath: String,
    ...
    PCRScheduleGenerator.generateSchedules(name, randomTracesPath, generatedSchedulesPath,
      optimization)
  }
   ...
   * r'->r that satisfy PCR are covered by traces.
   */
